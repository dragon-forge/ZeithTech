package org.zeith.tech.shadow.fabric;
...
public interface MutableQuadView
        extends QuadView
{
    static MutableQuadView getInstance()
    {
        return MutableQuadViewImpl.THREAD_LOCAL.get();
    }
    
    /**
     * Causes texture to appear with no rotation. Pass in bakeFlags parameter to
     * {@link #spriteBake(int, TextureAtlasSprite, int)}.
     */
     ...
     * replaced. Pass in bakeFlags parameter to {@link #spriteBake(int, TextureAtlasSprite, int)}.
     *
     * <p>
     ...
     * randomization and texture mapping scenarios. Results are different than what can be obtained via rotation and
     * both can be applied. Pass in bakeFlags parameter to {@link #spriteBake(int, TextureAtlasSprite, int)}.
     */
    int BAKE_FLIP_U = 8;
    ...
    /**
     * If non-null, quad is coplanar with a block face which, if known, simplifies or shortcuts geometric analysis that
     * might otherwise be needed. Set to null if quad is not coplanar or if this is not known. Also controls face
     * culling during block rendering.
     *
     * <p>
     * Null by default.
     *
     ...
     *
     * <p>
     * This is different than the value reported by {@link BakedQuad#getDirection()}. That value is computed based on
    ...
    /**
     * Enables bulk vertex data transfer using the standard Minecraft vertex formats. This method should be performant
     * whenever caller's vertex representation makes it feasible.
     *
     * <p>
     * Calling this method does not emit the quad.
     *
     * @deprecated Use {@link #fromVanilla(BakedQuad, RenderMaterial, Direction, int[], int)} which has better
     * encapsulation and removed outdated item flag
     */
    @Deprecated
     ...
     * Calling this method does not emit the quad.
     */
    MutableQuadView fromVanilla(BakedQuad quad, Direction cullFace);
    
    /**
     * Encodes an integer tag with this quad that can later be retrieved via {@link QuadView#tag()}. Useful for models
    ...
    /**
     * Adds a vertex normal. Models that have per-vertex normals should include them to get correct lighting when it
     * matters. Computed face normal is used when no vertex normal is provided.
     *
     * <p>
     * Renderer implementations should honor vertex normals for diffuse lighting - modifying vertex color(s) or packing
     * normals in the vertex buffer as appropriate for the rendering method/vertex format in effect.
     */
     ...
     * values are higher. Exposed for completeness but some rendering implementations with non-standard lighting model
     * may not honor it.
     */
    ...
    /**
     * Convenience: set sprite color for all vertices at once. Behavior for {@code spriteIndex > 0} is currently
     * undefined.
     */
     ...
     *
     * <p>
     * Only use this function if you already have a {@link Vec2}. Otherwise, see
     * {@link MutableQuadView#sprite(int, int, float, float)}.
     */
     ...
     * interface. Behavior for {@code spriteIndex > 0} is currently undefined.
     */
