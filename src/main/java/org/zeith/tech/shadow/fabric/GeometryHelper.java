...
public abstract class GeometryHelper
{
    ...
    public static boolean isQuadParallelToFace(Direction face, QuadView quad)
    {
        ...
        return equal(val, quad.posByIndex(1, i)) && equal(val, quad.posByIndex(2, i))
                && equal(val, quad.posByIndex(3, i));
    }
    
    /**
     * True if quad - already known to be parallel to a face - is actually coplanar with it. For compatibility with
    ...
    /**
     * Returns true if quad is truly a quad (not a triangle) and fills a full block cross-section. If known to be true,
     * allows use of a simpler/faster AO lighting algorithm.
     *
     * <p>
     * Does not check if quad is actually coplanar with the light face, nor does it check that all quad vertices are
     ...
     * corners outside the block to be at the corners.
     */
