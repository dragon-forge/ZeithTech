		...
		extends Item
{
	...
	public InteractionResult onItemUseFirst(ItemStack stack, UseOnContext context)
	{
		...
		{
			openMultimeter(context.getPlayer(), measurable);
			return InteractionResult.SUCCESS;
		}).orElse(InteractionResult.PASS);
	}
	
	public <T extends BlockEntity & IContainerTile> void openMultimeter(Player player, IEnergyMeasurable tile)
	{
		if(player instanceof ServerPlayer && tile != null && tile.getMeasurablePosition() != null)
			NetworkHooks.openScreen((ServerPlayer) player, forMeasurable(tile), buf -> buf.writeBlockPos(tile.getMeasurablePosition()));
	}
	
	public MenuProvider forMeasurable(IEnergyMeasurable tile)
	{
		return new SimpleMenuProvider((windowId, playerInv, player) -> new ContainerMultimeter(windowId, playerInv, new ContainerMultimeter.MultimeterData(tile, player.level.isClientSide)), getDescription());
	}
}