		...
		implements IRegisterListener, ICustomBlockItem
{
	...
	public static Supplier<IToolModifyHandler> HEVEA_LOG_STRIPPING = () -> ctx ->
	{
		if(ToolActions.AXE_STRIP == ctx.toolAction())
			return BlocksZT.STRIPPED_HEVEA_LOG.defaultBlockState()
	...
	{
		if(ToolActions.AXE_STRIP == ctx.toolAction())
			return BlocksZT.STRIPPED_HEVEA_WOOD.defaultBlockState()
					.setValue(DISLOCATED, ctx.state().getValue(DISLOCATED))
					.setValue(AXIS, ctx.state().getValue(AXIS));
		return null;
	};
	...
	public HeveaLogBlock(BlockBehaviour.Properties props)
	{
		super(props);
	...
	{
		if(isRandomlyTicking(state) && rng.nextInt(3) == 0)
		{
	...
	public HeveaLogBlock toolModify(IToolModifyHandler toolModify)
	{
		this.toolModify = toolModify;
		return this;
	}
	
	public HeveaLogBlock dropsSelf()
	{
		this.dropsSelf = true;
		return this;
	}
	
	public HeveaLogBlock addItemTags(List<TagKey<Item>> tags)
	{
		for(var tag : tags)
		{
			if(itemBlock != null)
	...
	public HeveaLogBlock addBlockTags(List<TagKey<Block>> tags)
	{
		for(var tag : tags)
